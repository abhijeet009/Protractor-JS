...
describe('Board page',function(){
   ...
   /* it('Add comment box',function(){
    ...
    it('writing using pen', async()=>{
      await element.findElement(By.css(".pen-tool .svg__icon")).click()
      // 5 | click | css=.tool-pen > .svg__icon |  | 
      await element.findElement(By.css(".tool-pen > .svg__icon")).click()
      // 6 | mouseDownAt | id=rectZoom | 2164.64111328125,7384.162109375 | 
      {
        const element = await element.findElement(By.id("rectZoom"))
        await element.actions({ bridge: true }).moveToElement(element).clickAndHold().perform()
      }
      // 7 | mouseMoveAt | id=rectZoom | 2164.64111328125,7384.162109375 | 
      {
        const element = await element.findElement(By.id("rectZoom"))
        await element.actions({ bridge: true }).moveToElement(element).perform()
      }
      // 8 | mouseUpAt | id=rectZoom | 2164.64111328125,7384.162109375 | 
      {
        const element = await element.findElement(By.id("rectZoom"))
        await element.actions({ bridge: true }).moveToElement(element).release().perform()
      }
      // 9 | click | id=rectZoom |  | 
      await element.findElement(By.id("rectZoom")).click()
      ...
      {
        const element = await element.findElement(By.id("svg-doc1091-kzwapv57ntl"))
        await element.actions({ bridge: true }).moveToElement(element).perform()
      }
